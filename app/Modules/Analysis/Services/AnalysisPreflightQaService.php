...
class AnalysisPreflightQaService
{
    ...
    private function studentChecks(Survey $mainSurvey, ?Survey $student): array
    {
        ...
        $checks[] = $this->check('student.question_order', 'Student question order valid', 'student_questionnaire', 'critical', $orderValid, 'Student questionnaire follows the approved A-H order.', 'Normalize Student Order in Builder.', route('admin.surveys.builder.index', ['survey' => $student]), 'Open Builder');
    ...
    private function lecturerChecks(Survey $mainSurvey, ?Survey $lecturer): array
    {
        ...
        return array_merge($checks, [
            ...
            $this->check('lecturer.question_order', 'Lecturer question order valid', self::SCOPE_LECTURER_QUESTIONNAIRE, 'critical', $orderValid, 'Lecturer questionnaire follows the approved consent/profile/content order.', 'Normalize Lecturer Order in Builder.', route('admin.surveys.builder.index', ['survey' => $lecturer]), 'Open Builder'),

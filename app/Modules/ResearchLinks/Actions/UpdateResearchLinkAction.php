...
class UpdateResearchLinkAction
{
     ...
     */
    public function handle(User $user, ResearchLink $researchLink, array $attributes, ?Request $request = null): ResearchLink
    {
        Gate::forUser($user)->authorize('update', $researchLink);

        $project = $this->projectFor($user, $attributes['research_project_id'] ?? null);
        $category = $this->category($attributes['category'] ?? $researchLink->category);
        ...
        $this->activityLogger->log('research_link.updated', $user, $project, $researchLink, $this->metadata($researchLink), $request);

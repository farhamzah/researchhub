...
class SurveyResponseExportRowBuilder
{
    ...
    {
        $response->loadMissing(['survey.project', 'respondent', 'answers']);

        $survey = $response->survey;
        $row = [
            'response_id' => $response->getKey(),
            'survey_id' => $survey->getKey(),
            'respondent' => $this->privacyService->display($response->respondent, $survey, $viewer),
    ...
    public function buildForSurvey(Survey $survey, ?User $viewer = null, bool $withIdentity = false): array
    {
        return $survey->responses()
            ->with(['survey.project', 'respondent', 'answers'])
            ->latest('submitted_at')
            ->get()

<article class="min-w-0 rounded-xl border border-slate-200 bg-slate-50 p-4" data-review-item>
    ...
    <div class="mt-4 grid min-w-0 gap-3 md:grid-cols-2">
        ...
        <label class="min-w-0 md:col-span-2"><span class="text-sm font-semibold">Redaksi revisi yang disarankan</span><textarea name="comments[item_{{ $question['id'] }}][suggested_revision]" rows="2" class="mt-1 block w-full min-w-0 rounded-lg border-slate-300">{{ old("comments.item_{$question['id']}.suggested_revision") }}</textarea></label>
    </div>
</article>

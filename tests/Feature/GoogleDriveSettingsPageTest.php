...
class GoogleDriveSettingsPageTest extends TestCase
{
    ...
    public function test_google_drive_settings_page_shows_safe_disconnected_state(): void
    {
        ...
        $this->actingAs($user)
            ...
            ->assertSee('Hubungkan MyRiset ke Google Drive')
            ->assertSee('Gunakan akun Google Drive Anda sendiri')
            ->assertSee('MyRiset tetap menjadi pusat workflow dan metadata')
            ->assertSee('Status Koneksi')
            ->assertSee('Kesiapan OAuth')
            ->assertSee('Redirect URI dan Scope')
            ->assertSee('Checklist Setup')
            ->assertSee('Folder Google Drive')
            ->assertSee('Hubungkan Google Drive dulu')
            ->assertSee('Belum terhubung')
            ->assertSee('Credential belum lengkap')
            ->assertSee('Belum dikonfigurasi')
            ->assertSee('Redirect URI terisi')
            ->assertSee('Lengkapi OAuth dulu')
            ->assertDontSee('/settings/drive/google/redirect')
            ->assertSee('Google Drive belum dikonfigurasi')
    ...
    public function test_google_drive_settings_page_shows_oauth_readiness_without_secret_value(): void
    {
        ...
        $this->actingAs($user)
            ...
            ->assertSee('Hubungkan MyRiset ke Google Drive')
            ->assertSee('Siap')
            ->assertSee('Client ID terisi')
            ->assertSee('Client Secret terisi')
            ->assertSee('Redirect URI terisi')
            ->assertSee('Nilai disembunyikan demi keamanan')
    ...
    public function test_google_drive_settings_page_shows_connected_metadata_without_tokens(): void
    {
        ...
        $this->actingAs($user)
            ...
            ->assertSee('Putuskan Koneksi')
            ->assertSee('Siapkan Folder MyRiset')
            ->assertSee('Google Drive sudah terhubung, tetapi folder MyRiset belum dibuat')
            ->assertSee('/settings/drive/google/disconnect')
            ->assertSee('/settings/drive/google/bootstrap-folders')
            ->assertSee('Putuskan Google Drive untuk pengguna ini?', false)
